...
// 	p, err := data.GetPollWithDetails(uint(pollId))
// 	if err != nil {
...
// 				generatedID, err := generateRandomString(16) // 16 bytes for a decent length
// 				if err != nil {
...
// 				if result := db.Where("question_id = ? AND voter_id = ?", currentQ.ID, voterID).Delete(&Vote{}); result.Error != nil {
...
// 					VoterID:    voterID,
// 				}
// 				if result := db.Create(&newVote); result.Error != nil {
...
// 					if result := db.Save(p); result.Error != nil { // Save poll status and index
...
// 						if result := db.Save(p); result.Error != nil { // Save poll status
// 							log.Printf("Error saving poll status to DB: %v", result.Error)
